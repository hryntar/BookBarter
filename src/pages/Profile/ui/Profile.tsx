...
export const Profile = () => { 
   ...
   return isLoading ? (
      ...
      <section className="mt-[80px] max-sm:mt-[20px] sm:px-10 mx-auto"> 
         {data ? (
            <ProfileUser login={data.login} email={data.email} phone={data.phone} rating={data.rating} image={data.image} /> 
         ): null}
         ...
         <div className="flex flex-col items-center">
            <div className="grid grid-cols-3 gap-10 max-2xl:grid-cols-2 max-lg:grid-cols-1">
               {data && data?.books.map((book) => (
                  <ProfileBook title={book.title} description={book.description} author={book.author} image={book.image} genres={book.genres} id={book.id} price={book.price} publishedBy={book.publishedBy} year={book.year} user={data} />
               ))}
            </div>
         </div>
      </section>
