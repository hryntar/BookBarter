import { Button, Navbar, NavbarBrand, NavbarContent, NavbarItem, NavbarMenuToggle, Tooltip } from "@nextui-org/react";
               ...
               {menuItems.map((item, index) => (
                  <NavbarItem key={index} className="mr-1">
                     <Tooltip showArrow color="primary" content={item.description}>
                        <Button onPress={() => navigate(item.link)} variant="light" radius="full" color="primary" isIconOnly aria-label={item.description}>
                           {item.icon}
                        </Button>
                     </Tooltip>
                  </NavbarItem>
